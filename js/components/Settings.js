    ...
    return (
      <div>
        <Modal closeOnRootNodeClick={false} dimmer="inverted" open={open} onClose={this.close}>
          ...
          <Modal.Content>
            <Modal.Description>
                <Grid.Row> Timeout  <Input id='ssmTimeout' type='number' onChange={this.handleTextChange} value={this.state.settings['ssmTimeout']} /></Grid.Row>
                &nbsp;
                <Grid.Row>
                  <Radio toggle label="Logging to S3" checked={this.state.settings['logToS3']}
                ...
                &nbsp;
                <Grid.Row><Radio toggle label="prompt에 hostname 보이기" checked={this.state.settings['terminalShowHostname']} 
          ...
          <Modal.Actions>
            ...
            <Button positive icon='checkmark' labelPosition='right' content="저장" onClick={this.saveSettings} />
          </Modal.Actions>
        </Modal>
      </div>
    );
