    ...
    return (
      <div>
        <Modal closeOnRootNodeClick={false} dimmer="inverted" open={open} onClose={this.close}>
          <Modal.Header>인증 방법</Modal.Header>
          <Modal.Content>
            <Modal.Description>
            </Modal.Description>
             <Grid.Row><Radio label="credentials file 가져오기" value="credFile" checked={authOptionSelected === "credFile"} onChange={(e, { value }) => {this.setState({authOptionSelected:value})}} /></Grid.Row>
             &nbsp;
          ...
          <Modal.Actions>
            ...
            <Button positive icon='checkmark' labelPosition='right' content="저장" onClick={this.saveChanges} />
          </Modal.Actions>
        </Modal>
      </div>
    );
